...
-- MOONHUB Find Trait Burner Module (as loadstring module)
-- Si el toggle global indica OFF, no ejecutar el módulo
if _G.FindTBActive == false then
    return {
        stop = function() print("[FindTB] Already stopped") end,
        status = function() return { active = false, entered = false, lastSelected = "Challenge1" } end
    }
end
-- Marcar ejecución activa para evitar doble arranque
...
-- ===================== MAIN FLOW ============================
task.spawn(function()
    -- No iniciar si el toggle global está apagado
...
-- === API de control expuesta por el módulo ===
return {
    stop = function()
        -- Apaga el ciclo y cancela búsquedas futuras
        _G.FindTBActive = false
        ENTERED = true -- fuerza salida de los bucles
